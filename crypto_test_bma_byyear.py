...
def main():
    ...
    query = (
        "UPDATE TableByYears SET current_price=%s, start_sum=%s, bal_margin=%s, total_sum = %s, duration = %s, comb_status = %s "
        "where id = %s")
    # print(query)
...
for sy in selyear:
    ...
    klines = pd.read_pickle("./data/crypto_btcusdt" + sy + "_" + per0 + ".pkl")
    startPrice = 0
    try:
    ...
    for oldOrd in oldOrdArr:
        for margin in marginArr:
            for step in stepArr:
                id = (str(sy) + "*y*" + "step" + str(step) + "oldord" +
                      str(oldOrd) + "margin" + str(margin) + "per" + str(per0))
                per = per0
                balUSD = balOverall / 2
                volUSD = balUSD / 20
                ...
                cursor = cnx.cursor(buffered=True)
                query = "SELECT comb_status from TableByYears where id = %s"
                cursor.execute(
                    query,
                    (id, ),
                )
                ...
                cursor = cnx.cursor(buffered=True)
                query = ("INSERT INTO TableByYears "
                         "(id, "
                         "sel_interval, "
                         "period, "
                         "step, "
                ...
                order = {
                    ...
                    "date":
                    pd.Timestamp(
                        date(int(sy), 1, 1).__add__(timedelta(days=1))),
                    "id":
                    0,
                    "vol":
                    0.0,
