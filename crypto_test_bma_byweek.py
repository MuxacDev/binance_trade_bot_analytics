...
def main():
    ...
    query = (
        "UPDATE TableByWeeks SET current_price=%s, start_sum=%s, bal_margin=%s, total_sum = %s, duration = %s, comb_status = %s "
        "where id = %s")
    # print(query)
...
from crypto_settings_bma_w import *
...
for sy in selyear:
    ...
    for sw in selweek:
        ...
        startPrice = 0
        try:
            df = df0[df0.index.isocalendar().week == sw]
            klines = klines0[klines0.index.isocalendar().week == sw]
        ...
        for oldOrd in oldOrdArr:
            for margin in marginArr:
                for step in stepArr:
                    orderDate = None
                    try:
                        orderDate = pd.Timestamp(
                            date.fromisocalendar(int(sy), sw,
                    ...
                    if w.__len__() < 5:
                        w = "0" + w
                    id = (str(sy) + "_" + w + "step" + str(step) + "oldord" +
                    ...
                    cursor = cnx.cursor(buffered=True)
                    query = "SELECT comb_status from TableByWeeks where id = %s"
                    cursor.execute(
                        query,
                        (id, ),
                    )
                    ...
                    cursor = cnx.cursor(buffered=True)
                    query = ("INSERT INTO TableByWeeks "
                             "(id, "
                             "sel_interval, "
                             "period, "
                             "step, "
