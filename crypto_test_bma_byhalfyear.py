...
def main():
    ...
    cnx = mysql.connector.connect(
        user="dbuser",
        password="password",
        host="171.159.117.102",
        database="TradeBotDb",
    )
    cursor = cnx.cursor(buffered=True)
    query = (
        "UPDATE TableByHalfYears SET current_price=%s, start_sum=%s, bal_margin=%s, total_sum = %s, duration = %s, comb_status = %s "
        "where id = %s")
    # print(query)
    ...
    cursor.execute(
        query,
        (
            str(currentBid).replace(",", "."),
            str(startBal).replace(",", "."),
            str(round((1 - (startBal / finBal)) * 100, 2)).replace(",", "."),
            str(finBal).replace(",", "."),
...
# ]
for sy in selyear:
    df0 = pd.read_pickle("./data/crypto_btcusdt" + sy + ".pkl")
    klines0 = pd.read_pickle("./data/crypto_btcusdt" + sy + "_" + per0 +
                             ".pkl")
    halfyearNumber = 0
    for shy in selhalfyear:
        halfyearNumber = halfyearNumber + 1
        df = pd.concat([
            df0[df0.index.month == shy[0]],
            df0[df0.index.month == shy[1]],
            df0[df0.index.month == shy[2]],
            df0[df0.index.month == shy[3]],
            df0[df0.index.month == shy[4]],
            df0[df0.index.month == shy[5]],
        ])
        klines = pd.concat([
            klines0[klines0.index.month == shy[0]],
            klines0[klines0.index.month == shy[1]],
            klines0[klines0.index.month == shy[2]],
            klines0[klines0.index.month == shy[3]],
            klines0[klines0.index.month == shy[4]],
            klines0[klines0.index.month == shy[5]],
        ])
        startPrice = 0
        try:
        ...
        for oldOrd in oldOrdArr:
            for margin in marginArr:
                for step in stepArr:
                    ...
                    if q.__len__() < 6:
                        q = "0" + q
                    id = (str(sy) + "_" + q + "step" + str(step) + "oldord" +
                          str(oldOrd) + "margin" + str(margin) + "per" +
                          str(per0))
                    per = per0
                    balUSD = balOverall / 2
                    volUSD = balUSD / 20
                    balBTC = round((balOverall - balUSD) / startPrice, 5)
                    volBTC = round(balBTC / 20, 5)
                    ...
                    cursor = cnx.cursor(buffered=True)
                    query = "SELECT comb_status from TableByHalfYears where id = %s"
                    cursor.execute(
                        query,
                        (id, ),
                    )
                    ...
                    cursor = cnx.cursor(buffered=True)
                    query = ("INSERT INTO TableByHalfYears "
                             "(id, "
                             "sel_interval, "
                             "period, "
                             "step, "
