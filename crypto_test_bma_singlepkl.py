...
def main(startPoint, finishPoint):
    global currentBid
    saveBTCbal = balBTC
    saveUSDbal = balUSD
    ...
    for nowt in df.index:
        if nowt < pd.Timestamp(startPoint) or nowt > pd.Timestamp(finishPoint):
            continue
    ...
    cursor = cnx.cursor()
    query = (
        "UPDATE MainTab SET total_sum = %s, comb_status = %s "
        "where start_point=%s and "
        "finish_point=%s and "
        "period = %s and "
        "step = %s and "
        # "vol_btc = %s and "
...
from crypto_settings_bma_singlepkl import *

df = pd.read_pickle("./data/crypto_btcusdt.pkl")
klines = pd.read_pickle("./data/crypto_btcusdt_" + per0 + ".pkl")
...
for oldOrd in oldOrdArr:
    for margin in marginArr:
        for step in stepArr:
            ...
            cursor = cnx.cursor()
            query = (
                "SELECT comb_status from MainTab "
                "where start_point=%s and "
                "finish_point=%s and "
                "period=%s and "
                "step=%s and "
            ...
            for nowt in df.index:
                if str(nowt)[:10] == str(pd.Timestamp(startPoint))[:10]:
                    startPrice = (df.high.to_numpy()[0] +
                                  df.low.to_numpy()[0]) / 2
            ...
            order = {
                "type": "BUY",
                "date": startPoint.__add__(timedelta(days=1)),
                "id": 0,
                "vol": 0.0,
                "price": 0.0,
            }
